...
class Engine():
    ...
    def echo(self, entry, display = True, log = True, style = 0):
        ...
        if log and self.logFileName != "":
            ...
            logfile = open(os.path.join(self.logfilespath, self.logFileName), "a", encoding=self.encoding)
            ...
            columnHeading.strip() != "" and columnHeading.strip()[0] != "#"):
                try:
                    ...
                    if self.debug:
                        ...
                        scriptErrorMsg = "Invalid column heading '%s' (column %s). %s" % (columnHeading, n + 1, str(exceptionValue))
    ...
    def getEquellaVersion(self):
        ...
        try:
            ...
            if self.debug:
                ...
                self.echo(''.join(str(line) for line in traceback.format_exception(exceptionType, exceptionValue, exceptionTraceback)))
    ...
    def runImport(self, owner, testOnly=False):
        ...
        try:
            ...
            # Get Collection UUID
            self.echo(time.strftime("%H:%M:%S: ", time.localtime(time.time())) + "Target collection: '" + str(self.collection)  + "'...")
            ...
            if self.collection in self.collectionIDs.keys():
                ...
                raise Exception("Collection '" + str(self.collection) + "'" + " not found")
            try:
                if not os.path.isdir(self.csvFilePath):
                    self.echo(time.strftime("%H:%M:%S: ", time.localtime(time.time())) + "Parsing CSV file (" + str(self.csvFilePath) + ")...")
                else:
                    self.echo(time.strftime("%H:%M:%S: ", time.localtime(time.time())) + "WARNING: No CSV specified. CSV path is " + str(self.csvFilePath))
                ...
                self.echo(time.strftime("%H:%M:%S: ", time.localtime(time.time())) + "DEBUG: rowErrorColumn = %d" % rowErrorColumn)
                ...
                self.echo(time.strftime("%H:%M:%S: ", time.localtime(time.time())) + "ERROR: " + str(exact_error).strip() + errorString, style=2)
            ...
            self.echo(time.strftime("%H:%M:%S: ", time.localtime(time.time())) + "ERROR whilst trying to connect: " + str(exact_error).strip() + errorString, style=2)
    ...
    def csvParse(self,
        ...
        if not self.export:
            if itemIdColumn != -1 or rowErrorColumn != -1:
                ...
                self.echo(time.strftime("%H:%M:%S: ", time.localtime(time.time())) + "DEBUG: receiptFilename = %s" % receiptFilename)
        ...
        if self.debug:
            self.echo(time.strftime("%H:%M:%S: ", time.localtime(time.time())) + "Absolute attachments basepath is " + str(self.absoluteAttachmentsBasepath))
        ...
        if self.startScript.strip() != "" and not self.export:
            try:
                ...
                if self.debug:
                    ...
                    exceptionType, exceptionValue, exceptionTraceback = sys.exc_info()
                    formattedException = "".join(str(line) for line in traceback.format_exception_only(exceptionType, exceptionValue))[:-1]
        ...
        if not self.export:
            ...
            for rowCounter in scheduledRows:
                ...
                if rowCounter < len(self.csvArray):
                    ...
                    if rowErrorColumn != -1:
                        self.echo("  DEBUG: rowData length = %d, rowErrorColumn = %d" % (len(rowData), rowErrorColumn))
                        if rowErrorColumn < len(rowData):
                            ...
                            while len(rowData) <= rowErrorColumn:
                                rowData.append("")
                            rowData[rowErrorColumn] = rowError
                            self.echo("  DEBUG: Extended rowData to length %d" % len(rowData))
                        
                        if rowError != "":
                            self.echo("  Row error captured: %s" % rowError)
                            self.echo("  DEBUG: Storing error at index %d in rowData" % rowErrorColumn)
                            self.echo("  DEBUG: rowData[%d] = %s" % (rowErrorColumn, rowData[rowErrorColumn]))
                            self.echo("  DEBUG: csvArray[%d][%d] = %s" % (rowCounter, rowErrorColumn, self.csvArray[rowCounter][rowErrorColumn]))
                    else:
            ...
            if itemIdColumn != -1 or rowErrorColumn != -1:
                ...
                self.echo(time.strftime("%H:%M:%S: ", time.localtime(time.time())) + "DEBUG: Writing to %s" % receiptFilename)
                ...
                for i in range(1, len(self.csvArray)):
                    ...
                    self.echo(time.strftime("%H:%M:%S: ", time.localtime(time.time())) + "DEBUG: Writing row %d with %d columns: %s" % (i, len(row_to_write), str(row_to_write)))
                    writer.writerow(row_to_write)
                f.close()
                self.echo(time.strftime("%H:%M:%S: ", time.localtime(time.time())) + "DEBUG: Receipt file closed")
        ...
        if (not testOnly or rowErrorColumn != -1) and not os.path.isdir(self.csvFilePath):
            ...
            with open(self.csvFilePath, "w", encoding=self.encoding, newline='') as f:
                ...
                f.flush()
                os.fsync(f.fileno())
            self.echo(time.strftime("%H:%M:%S: ", time.localtime(time.time())) + "CSV file updated successfully")
        ...
        if resourceItemUuid[0] == "[" and resourceItemUuid[-1] == "}":
            ...
            if collSplitPoint != -1:
                ...
                if collection in collectionIDs:
                    ...
                    raise Exception("Collection specified not found: " + str(collection))
            ...
            try:
                ...
                if ownerColumn != -1 and ownerUsername != "":
                    ...
                    if len(matchingUserNodes) > 0:
                        ...
                        for userNode in matchingUserNodes:
                            try:
                                userUUID = userNode.getElementsByTagName("uuid")[0].firstChild.nodeValue
                                userInfo = self.tle.getUser(userUUID)
                                username = userInfo.getNode("username")
                                if username and username.strip().lower() == ownerUsername.lower():
                                    ownerID = userUUID
                        ...
                        ownerColumn = -1
                        self.echo("  '%s' not found so ignoring." % (ownerUsername))
                    else:
                        ...
                        if len(matchingUserNodes) > 0:
                            ...
                            for userNode in matchingUserNodes:
                                try:
                                    ...
                                    userInfo = self.tle.getUser(userUUID)
                                    username = userInfo.getNode("username")
                                    if username and username.strip().lower() == specifiedCollaborator.strip().lower():
                                        collaboratorID = userUUID
                ...
                if "DELETE" in commandOptions:
                    ...
                    for n in range(0, len(meta)):
                        ...
                        if self.currentColumns[n][self.COLUMN_DATATYPE] == self.URLS:
                            ...
                            # delete all url metadata targets if first occurence (but not in append attachments mode)
                        ...
                        if self.currentColumns[n][self.COLUMN_DATATYPE] == self.ATTACHMENTLOCATIONS:
                            ...
                            for i in range(len(values)):
                                if values[i].strip() != "":
                                    ...
                                    if not os.access(filepath, os.R_OK):
                                        raise Exception("Cannot read file: '%s' (permission denied)" % filepath)
                                    ...
                                    try:
                                        ...
                                        raise Exception("Cannot get file size for '%s': %s" % (filepath, str(e)))
                                        ...
                                        if (os.path.splitext(filename)[1].upper() == ".ZIP"):
                                            ...
                                            if ("IMS" in commandOptions or "SCORM" in commandOptions or "AUTO" in commandOptions):
                                                if ("imsmanifest.xml" in zfobj.namelist()):
                                                    imsmanifest = PropBagEx(zfobj.read("imsmanifest.xml").decode("utf-8"))
                                    ...
                                    if attachmentType == self.attachmentTypeFile:
                                        ...
                                        # Get the custom XPath from column heading if it's not a standard column
                                        customXPath = None
                                        if self.columnHeadings[n].strip() != "" and not self.columnHeadings[n].startswith("#"):
                                            customXPath = self.columnHeadings[n]
                                        
                                        if attachmentLinkName != "":
                                            # add start page with provided attachment link name
                                            item.addStartPage(attachmentLinkName, filename, filesize, attachmentUUID, thumbnail, self.appendAttachments, customXPath)
                                        else:
                                            # add start page using filename as attachment link
                                            item.addStartPage(filename, filename, filesize, attachmentUUID, thumbnail, self.appendAttachments, customXPath)
                                        
                                        # add metadata target for attachment
                                        # NOTE: If customXPath was provided, addStartPage already handled storing the UUID there
                                        # So only do this if customXPath was NOT provided
                        ...
                        if self.currentColumns[n][self.COLUMN_DATATYPE] == self.CUSTOMATTACHMENTS and meta[n].strip() != "":
                            ...
                            xmlAttachmentElements = PropBagEx(xmlAttachmentElementsFragmentString, encoding=self.encoding)
                        ...
                        # process metadata field (skip if this is a Custom Attachments column)
                        # Detect Custom Attachments by checking if content contains <attachment> tag (XML fragment)
                        isCustomAttachmentsColumn = (self.currentColumns[n][self.COLUMN_DATATYPE] == self.CUSTOMATTACHMENTS or 
                                                   (meta[n].strip().lower().startswith("<attachment") and "<type>" in meta[n].lower()))
                        
                        if self.currentColumns[n][self.COLUMN_DATATYPE] == self.METADATA and not isCustomAttachmentsColumn:
                            ...
                            if self.currentColumns[n][self.COLUMN_XMLFRAGMENT] == "YES":
                                ...
                                if meta[n].strip() != "":
                                    ...
                                    # append xml fragment
                                    parentNodes = item.prop.getNodes(self.columnHeadings[n], False)
                            ...
                            # simple metadata, not an XML fragment 
                                ...
                                for i in range(len(newValues)):
                                    ...
                                    if newValues[i].strip() != "":
                                        ...
                                        item.getXml().createNode(self.columnHeadings[n], str(newValues[i].strip()))
                            ...
                            if ownerColumn != -1 and not createNewVersion and not createNewItem and ownerID != "" and ownerUsername != "":
                                ...
                                if item.getXml().getNode("item/owner") != ownerID:
                                    try:
                                        ...
                                        self.echo(time.strftime("%H:%M:%S: ", time.localtime(time.time())) + "ERROR: %s%s" % (str(exactError), str(errorDebug)), style=2)
                ...
                if failCount < self.maxRetry and ("(10054)" in exactError or "(10060)" in exactError or "(10061)" in exactError or "(104)" in exactError):
                    ...
                    if n != -1 and not attemptingUpload:
                        actionError =  " parsing column %s '%s'" % (n + 1, str(self.columnHeadings[n]))
                    if attemptingUpload:
                        actionError =  " uploading file"
                        attemptingUpload = False

                    self.echo(time.strftime("%H:%M:%S: ", time.localtime(time.time())) + "ERROR%s: %s%s" % (str(actionError), str(exactError), str(errorDebug)), style=2)
                    ...
                    return savedItemID, savedItemVersion, sourceIdentifier, meta, "ERROR%s: %s%s" % (str(actionError), str(exactError), str(errorDebug))
    ...
    def exportCSV(self,
                        ...
                        if collectionColumn != -1:
                            ...
                            if collectionName != "":
                                if collectionName in collectionIDs:
                                    ...
                                    raise Exception("'" + str(collectionName) + "' collection not found")
                        itemFound = False
        ...
        self.echo(time.strftime("%H:%M:%S: ", time.localtime(time.time())) + "DEBUG: testOnly=%s, csvFilePath=%s, is_dir=%s" % (testOnly, self.csvFilePath, os.path.isdir(self.csvFilePath)))
        # Write back to original CSV if Row Error column exists (even in test mode) or if not test mode
        if (not testOnly or rowErrorColumn != -1) and not os.path.isdir(self.csvFilePath):
            self.echo(time.strftime("%H:%M:%S: ", time.localtime(time.time())) + "Writing %d rows to CSV file: %s" % (len(self.csvArray), self.csvFilePath))
            self.echo(time.strftime("%H:%M:%S: ", time.localtime(time.time())) + "DEBUG: Header row: %s" % str(self.csvArray[0]))
            ...
            with open(self.csvFilePath, "w", encoding=self.encoding, newline='') as f:
                ...
                f.flush()
                os.fsync(f.fileno())
            self.echo(time.strftime("%H:%M:%S: ", time.localtime(time.time())) + "CSV file written successfully (file size: %d bytes)" % os.path.getsize(self.csvFilePath))
    ...
    def exportItem(self,
        ...
        self.echo("DEBUG: exportItem called for itemID=%s, itemVersion=%s" % (itemID, itemVersion))
        ...
        self.echo("DEBUG: Testing xpath 'item/@id': %s" % str(itemXml.getNode("item/@id")))
        self.echo("DEBUG: Testing xpath 'item/@version': %s" % str(itemXml.getNode("item/@version")))
        self.echo("DEBUG: Testing xpath 'item/name': %s" % str(itemXml.getNode("item/name")))
        ...
        for n in range(0, len(self.columnHeadings)):
            ...
            self.echo("DEBUG: Processing column %d: '%s', datatype=%s" % (n, self.columnHeadings[n], self.currentColumns[n][self.COLUMN_DATATYPE]))
            ...
            self.columnHeadings[n].strip() != "" and self.columnHeadings[n].strip()[0] != "#"):
                ...
                self.echo("DEBUG: Using xpath: '%s' (original: '%s')" % (xpath, self.columnHeadings[n]))
            ...
            if self.currentColumns[n][self.COLUMN_DATATYPE] == self.METADATA:
                                ...
                                if attachment.getNode("@type") == "local":
                                    if filename.find("/") == -1:
                                        ...
                                        if rootFolder.endswith(".zip"):
                                            ...
                                            fileUrl = itemUrl + "_zips/" + urllib.parse.quote(zipfilename)
                                    ...
                                    fileUrl = itemUrl + "_SCORM/" + urllib.parse.quote(filename)
                ...
                collName = next((key for key,value in collectionIDs.items() if value==collID))
                cellValues = [collName]
            ...
            self.echo("DEBUG: Column %d cellValues=%s, cellValue='%s'" % (n, str(cellValues), cellValue))
            ...
            if len(cellValues) > 0:
                ...
                self.echo("DEBUG: Column %d has no cellValues, rowData[%d] remains empty" % (n, n))
