...
SOAP_PARAMETER = (
    '<ns1:%(name)s xsi:type="%(type)s"%(arrayType)s>%(value)s</ns1:%(name)s>'
)

STANDARD_INTERFACE = {
    "url": "services/SoapService51",
    "namespace": "http://soap.remoting.web.tle.com",
}
...
def generate_soap_envelope(name, params, ns, token=None):
    ...
    return SOAP_REQUEST % {
        ...
        "header": (
            ...
            else SOAP_HEADER_TOKEN % {"token": token}
        ),
    }
...
def generateToken(username, sharedSecretId, sharedSecretValue):
    ...
    return "%s:%s%s:%s" % (
        ...
        binascii.b2a_base64(
            hashlib.md5(
                (username + sharedSecretId + seed + sharedSecretValue).encode()
            ).digest()
        ),
    )
...
class TLEClient:
    ...
    def __init__(
        ...
        if sso:
            self.sessionid = self._createSoapSessionFromToken(
                createSSOToken(username, password)
            )
        else:
    ...
    def _call(
        ...
        args,
        returns=1,
        facade=SOAP_ENDPOINT_V41,
        ns="http://soap.remoting.web.tle.com",
    ):
        try:
            headers = {}
            ...
            self._log_request(name, args, endpointUrl, headers, wsenvelope, ns)

            # make request
            request = urllib.request.Request(
                endpointUrl, wsenvelope.encode("utf-8"), headers
            )
            ...
            # Store info before closing response
            response_info = response.info()
            response.close()

            added_cookie = []
            for cookie_name in self._cookieJar:
                c_name = cookie_name.upper().split("=")[0].strip()
                if ";" in c_name:
                    c_name = c_name.split(";")[1].strip()
                added_cookie.append(c_name)

            self._process_response_cookies(response_info, added_cookie)
            self._log_response(response_info.as_string(), s)
        ...
        return self._parse_soap_response(s, returns)

    def _log_request(self, name, args, endpointUrl, headers, wsenvelope, ns):
        ...
        if name not in ["uploadFile"] or len(str(args)) < 1000:
            ...
            printableArgs = (
                "(%s, %s, ('%s', '%s', <%s characters of base64 data>), %s)"
    ...
    def _handle_http_error(self, e):
        ...
        httpError = (
            ...
            if hasattr(e, "reason") and isinstance(e.reason, bytes)
        ...
        if httpErrorBody != "":
            try:
                ...
                faultstring = (
                    errordom.firstChild.getElementsByTagName("soap:Body")[0]
    ...
    def _log_communication_end(self):
        self.owner.echo("-------------------------------------------------------------")
    ...
    def _parse_soap_response(self, s, returns):
        try:
            dom = parseString(s)
        except:
            if self.owner.networkLogging:
        ...
        if len(
            ...
            raise Exception(
                "Server returned following SOAP error: %s" % dom.toprettyxml()
            )
        elif returns:
            returnValue = dom.firstChild.getElementsByTagName("soap:Body")[
                0
    ...
    def _process_response_cookies(self, response_info, known_cookie_names):
        """
        Extracts and stores valid cookies from response headers.
        Filters out reserved keywords (path, domain, etc) and duplicates.
        This is separated to conform with Single Responsibility patterns.
        """
        ...
        cookie_parts = (
            part.strip() for header in headers if header for part in header.split(",")
        )
        for cookie in cookie_parts:
            ...
            name = (
                key_segment.split(";")[1].strip() if ";" in key_segment else key_segment
            )
    ...
    def _createSoapSessionFromToken(self, token):
        result = self._call("loginWithToken", (("token", "xsd:string", token),))
        ...
        while retry:
            try:
                ...
                if hasattr(err, "reason"):
                    reason_str = (
                        err.reason.decode("utf-8")
                        if isinstance(err.reason, bytes)
                        else str(err.reason)
                    )
    ...
    def _newItem(self, itemdefid):
        result = self._call("newItem", (("itemdefid", "xsd:string", itemdefid),))
    ...
    def newVersionItem(self, itemid, version, copyattachments=True):
        rv = NewItemClient(
            self,
            self.owner,
            self._newVersionItem(itemid, str(version), copyattachments),
            debug=self.debug,
        )
        return rv
    ...
    def editItem(self, itemid, version, copyattachments):
        ...
        rv = NewItemClient(
            ...
            dom,
            newversion=0,
            copyattachments=(copyattachments == "true"),
            debug=self.debug,
        )
        return rv
    ...
    def setOwner(self, itemid, itemversion, ownerid):
        result = self._call(
            ...
            1,
            facade=SOAP_ENDPOINT_V51,
        )

    def setOwnerByUsername(
        self, itemID, version, username, saveNonexistentUsernamesAsIDs=False
    ):
    ...
    def addSharedOwners(
        ...
        saveNonexistentUsernamesAsIDs=False,
    ):
    ...
    def getUser(self, userId):
        result = self._call(
            "getUser", (("userId", "xsd:string", userId),), 1, facade=SOAP_ENDPOINT_V51
        )
        return PropBagEx(value_as_string(result))
...
class NewItemClient:
    def __init__(
        self, parClient, owner, newDom, newversion=0, copyattachments=1, debug=False
    ):
        self.debug = debug
        self.owner = owner
    ...
    def attachFile(self, path, attachment, show_status=None, chunk_size=(1024 * 2048)):
        """
        Orchestrates the file upload process in chunks and propagates UI pauses.
        Focuses on flow control rather than implementation details.
        This function has been refactored to remove deep arrow antipatterns.
        """
        ...
        try:
            ...
            for i, chunk in enumerate(self.read_in_chunks(attachment, chunk_size)):
                ...
                self.parClient._uploadFile(
                    self.stagingid, path, encoded_chunk, first_chunk_str
                )

                # 3. Update State
                uploaded_bytes += len(chunk)
                self._notify_progress(
                    show_status, "Uploading...", uploaded_bytes, file_size
                )
    ...
    def _notify_progress(self, prefix, state, current, total):
        ...
        if self.debug:
            ...
            status_line = (
                f"{message}{percent}%" if current < total else f"{message}Done"
            )
    ...
    def addStartPage(
        ...
        if customXPath and uuid != "":
            targetPath = (
                customXPath.replace("/xml/", "")
                if customXPath.startswith("/xml/")
                else customXPath
            )
    ...
    def addStartPageWithZipAttribute(
        self, description, path, size=1024, uuid="", zipParentUUID=""
    ):
    ...
    def forceUnlock(self):
        self.parClient._forceUnlock(
            self.getUUID(), self.getVersion(), self.getItemdefUUID()
        )

    def cancelEdit(self):
        self.parClient._cancelEdit(
            self.getUUID(), self.getVersion(), self.getItemdefUUID()
        )

    def delete(self):
        self.parClient._deleteItem(
            self.getUUID(), self.getVersion(), self.getItemdefUUID()
        )
    ...
    def _createNewNodes(self, xpath, onlyOne=False):
        ...
        while rhs != "":
            ...
            if (
                not missingParents
                and rhs != ""
                and len(self.getNodes("/".join(xpathParts))) != 0
            ):
                i += 1
            else:
                missingParents = True
...
class XPath:
    ...
    def validateXpath(self, xpath):
        self._selectNodes(
            xpath, False, self.testNode, True, self.debugLevel, self.debugLevel
        )
        return True
    ...
    def _selectNodes(self, xpath, asStrings, curNode, validateOnly=False, dl=0, sd=0):
        if dl > 1:
            print(
                ...
                "".ljust(sd),
                "_selectNodes(<%s>, %s%s)"
                % (curNode.nodeName, xpath, ", validateOnly" if validateOnly else ""),
            )

        step, remainingXpath, delimiter = self.splitFirstOuter(
            xpath, ["/"], False, dl=dl, sd=sd + 1
        )
        ...
        if validateOnly:
            if stepNodename not in ["", ".", "..", "*", "text()", "node()"]:
                ...
                if nodename not in ["*", "node()"]:
                    if (
                        not all(c in ascii_letters + "-_.0123456789" for c in nodename)
                        or nodename[0] in "-_.0123456789"
                    ):
                ...
                if stepPredicate != "":
                    self.evaluateCondition(
                        stepPredicate, curNode, validateOnly, 0, 0, dl=dl, sd=sd + 1
                    )

            # if more xpath left recurse
            if remainingXpath != "":
                self._selectNodes(
                    remainingXpath, False, curNode, validateOnly, dl=dl, sd=sd + 1
                )
            ...
            if dl >= 1:
                print(
                    " DEBUG",
                    "".ljust(sd),
                    "_selectNodes(<%s>, %s) -> %s" % (curNode.nodeName, xpath, "VALID"),
                )
            return []
                ...
                if curNode.hasAttribute(attrNodename):
                    ...
                    for key in curNode.attributes.keys():
                        ...
                        if stepPredicate == "":
                            ...
                            predicateMatch = self.evaluateCondition(
                                stepPredicate,
                                curNode.attributes[key],
                                validateOnly,
                                i,
                                childNodeCount,
                                dl=dl,
                        ...
                        if predicateMatch:
                            if remainingXpath != "":
                                if remainingXpath.startswith("/"):
                                    ...
                                    matchingNodes += self.queryAllChildElements(
                                        remainingXpath, childNode, validateOnly
                                    )

                                # more relative xpath remaining so recurse _selectNodes() and include results for returning
                                matchingNodes += self._selectNodes(
                                    remainingXpath,
                                    asStrings=False,
                                    curNode=childNode,
        ...
        # check if empty string
            ...
            if remainingXpath != "":
                ...
                if remainingXpath.startswith("/"):
                    ...
                    remainingXpath = remainingXpath[1:]
                    matchingNodes += self.queryAllChildElements(
                        remainingXpath, curNode, validateOnly
                    )
        ...
        if dl >= 1:
            print(
                ...
                "".ljust(sd),
                "_selectNodes(<%s>, %s) -> %s"
                % (curNode.nodeName, xpath, returnValues),
            )
        return returnValues
    ...
    def splitFirstOuter(self, string, delimiters, reverse=False, dl=0, sd=0):
        ...
        if dl >= 7:
            ...
            print(
                " DEBUG",
                "".ljust(sd),
                "splitFirstOuter('%s', %s) -> %s"
                % (string, delimiters, (lhs, rhs, foundDelimiter)),
            )
        return lhs.strip(), rhs.strip(), foundDelimiter.strip()

    def evaluateCondition(
        self, statement, curNode, validateOnly, nodePosition, sibCount, dl=0, sd=0
    ):
        if dl > 2:
            print(" DEBUG", "".ljust(sd), "evaluateCondition(%s)" % (statement))
        lhs, rhs, operator = self.splitFirstOuter(
            statement, [" and ", " or "], False, dl, sd + 1
        )
        ...
        if lhs.strip()[0] == "(":
            ...
            lhsResult = self.evaluateCondition(
                lhs.strip()[1:i],
                curNode,
                validateOnly,
                nodePosition,
            ...
            # process statement
            lhsResult, lhsResultType = self.evaluateComparison(
                lhs, curNode, validateOnly, nodePosition, sibCount, dl, sd + 1
            )
        ...
        if rhs != "":
            ...
            rhsResult = self.evaluateCondition(
                rhs, curNode, validateOnly, nodePosition, sibCount, dl=dl, sd=sd + 1
            )
            if operator == "and":
        ...
        if dl >= 2:
            print(
                " DEBUG",
                "".ljust(sd),
                "evaluateCondition(%s) -> %s" % (statement, result),
            )
        return result

    def evaluateComparison(
        self, string, curNode, validateOnly, nodePosition, sibCount, dl=0, sd=0
    ):
        if dl > 3:
            print(" DEBUG", "".ljust(sd), "evaluateComparison(%s)" % (string))
        lhs, rhs, operator = self.splitFirstOuter(
            string, ["=", "!=", "<", ">", "<=", ">="], False, dl=dl, sd=sd + 1
        )
        lhsResult, lhsResultType = self.evaluateStatement(
            lhs, curNode, validateOnly, nodePosition, sibCount, dl, sd + 1
        )

        if rhs != "":
            rhsResult, rhsResultType = self.evaluateStatement(
                rhs, curNode, validateOnly, nodePosition, sibCount, dl, sd + 1
            )
            ...
            if lhsResultType == "NODE_SET" and rhsResultType == "NODE_SET":
                for lhsValue in lhsResult:
                    for rhsValue in rhsResult:
                        result = (
                            self.compareValues(
                                operator, lhsValue, "STRING", rhsValue, "STRING", dl, sd
                            ),
                            "BOOLEAN",
                        )
                        if result[0]:
        ...
        if dl >= 3:
            print(
                " DEBUG",
                "".ljust(sd),
                "evaluateComparison(%s) -> %s" % (string, result),
            )
        return result

    # function for evaluating against different operators
    def compareValues(
        self, operator, lhsValue, lhsValueType, rhsValue, rhsValueType, dl=0, sd=0
    ):
        result = False
        ...
        if dl >= 4:
            print(
                " DEBUG",
                "".ljust(sd),
                "compareValues(%s %s %s) -> %s"
                % (lhsValue, operator, rhsValue, result),
            )
        return result
    ...
    def evaluateStatement(
        ...
        if rhs.strip()[0] == "(":
            ...
            rhsResult, rhsResultType = self.evaluateValue(
                rhs, curNode, validateOnly, nodePosition, sibCount, dl, sd + 1
            )
        ...
        if lhs != "":
            ...
            if operator == "+":
                if lhsResultType == "NUMBER" and rhsResultType == "NUMBER":
                    ...
                    raise Exception(
                        "Can only use (+) operator for numbers not %s and %s '%s'"
                        % (lhsResultType, rhsResultType, statement)
                    )
            else:
                if lhsResultType == "NUMBER" and rhsResultType == "NUMBER":
                    result = lhsResult - rhsResult, "NUMBER"
        ...
        if dl >= 5:
            print(
                " DEBUG",
                "".ljust(sd),
                "evaluateStatement(%s) -> %s" % (statement, result),
            )
        return result

    def evaluateValue(
        self, value, curNode, validateOnly, nodePosition, sibCount, dl=0, sd=0
    ):
        if dl > 5:
        ...
        try:
            ...
            # string
            if (value.strip()[0] == '"' and value.strip()[-1] == '"') or (
                value.strip()[0] == "'" and value.strip()[-1] == "'"
            ):
                evaluation = value.strip()[1:-1], "STRING"
            # true()
            elif value.strip().startswith("true("):
                parameters = self.getFunctionParameters(
                    value,
                    [],
                    curNode,
                    validateOnly,
            ...
            # string()
                ...
                parameters = self.getFunctionParameters(
                    value,
                    ["BOOLEAN|STRING|NUMBER|NODE_SET"],
                    curNode,
                    validateOnly,
                    nodePosition,
                    sibCount,
                ...
                evaluation = (
                    self.convertToString(parameters[0][0], parameters[0][1]),
                    "STRING",
                )
            # upper-case()
            ...
            # substring()
                ...
                parameters = self.getFunctionParameters(
                    value,
                    ["BOOLEAN|STRING|NUMBER|NODE_SET", "NUMBER", "NUMBER"],
                    curNode,
                    validateOnly,
                    nodePosition,
                    sibCount,
            ...
            # string-length()
                ...
                if len(parameters) == 0:
                    if curNode.firstChild != None:
                        string = self.convertToString(
                            curNode.firstChild.nodeValue, "STRING"
                        )
                        if string != None:
                    ...
                    evaluation = (
                        len(self.convertToString(parameters[0][0], parameters[0][1])),
                        "NUMBER",
                    )
            # concat()
        ...
        if dl >= 5:
            print(
                " DEBUG", "".ljust(sd), "evaluateValue(%s) -> %s" % (value, evaluation)
            )
        return evaluation
    ...
    def getFunctionParameters(
        ...
        if dl > 6:
            print(
                " DEBUG",
                "".ljust(sd),
                "getFunctionParameters(%s, %s)" % (string, paramTypes),
            )
        ...
        while parameterString != "":
            param, parameterString, operator = self.splitFirstOuter(
                parameterString, [","], False, dl=dl, sd=sd + 1
            )
        ...
        if len(parameters) < minParams or len(parameters) > len(paramTypes):
            if len(paramTypes) == 0:
                ...
                msg = "Expecting between %s and %s parameters" % (
                    minParams,
                    len(paramTypes),
                )
        ...
        for i, parameter in enumerate(parameters):
            ...
            if [pt for pt in allowedTypes if pt in ["NUMBER", "STRING", "NODE_SET"]]:
                ...
                parameters[i] = self.evaluateStatement(
                    parameter[0],
                    curNode,
                    validateOnly,
                    nodePosition,
                ...
                if parameters[i][1] not in allowedTypes:
                    raise Exception(
                        "Incorrect parameter type %s in '%s' expecting one of the following %s"
                        % (parameters[i][1], string, allowedTypes)
                    )
        ...
        if dl >= 6:
            print(
                " DEBUG",
                "".ljust(sd),
                "getFunctionParameters(%s, %s) -> %s"
                % (string, paramTypes, parameters),
            )
        return parameters
